...
function ProfilePage({ show, hideSelf }) {
  ...
  return pageTransitions.map(({ item, key, props: pageProps }) => item && (
    <SidePage key={key} style={pageProps} onClick={handleClick} iswide={isWide ? 1 : 0}>
      {
        menuTransitions.map(({ item: menuItem, key: menuKey, props: menuProps }) => menuItem && (
          <SideMenu key={menuKey} style={menuProps} ref={(el) => { menu = el; }}>
            <UserAvatar />
            <UserName />
            <Followers />
