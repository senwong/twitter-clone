...
Mock.mock(/searchhot/, () => {
  ...
  res.users = Array(userNum)
    .fill(null)
    .map(() => getUserInfo());
  res.tweets = Array(userNum)
    .fill(null)
    .map(() => getTweet());
  return res;
});
Mock.mock(/recommendscreenname/, () =>
  Array(5)
    .fill(null)
    .map(() => Random.string("lower", 4, 14))
);
